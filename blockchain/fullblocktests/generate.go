...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: acmutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := acmutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*acmutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, acmutil.NewTx(tx))
	}
...
func additionalCoinbase(amount acmutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee acmutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee acmutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee acmutil.Amount) *wire.MsgTx {
