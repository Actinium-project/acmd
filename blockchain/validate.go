...
const (
	...
	baseSubsidy = 50 * acmutil.SatoshiPerBitcoin
)
...
func IsCoinBase(tx *acmutil.Tx) bool {
...
func IsFinalizedTransaction(tx *acmutil.Tx, blockHeight int32, blockTime time.Time) bool {
	msgTx := tx.MsgTx()
...
func CalcBlockSubsidy(height int32, chainParams *chaincfg.Params) int64 {
	...
	if height < chainParams.ACMZawyLWMAHeight {
		if height >= chainParams.GPUSupportHeight {
			return baseSubsidy - (10 * acmutil.SatoshiPerBitcoin)
		}
	}
...
func CheckTransactionSanity(tx *acmutil.Tx) error {
	...
	for _, txOut := range msgTx.TxOut {
		...
		if totalSatoshi > acmutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
func checkProofOfWork(header *wire.BlockHeader, powLimit *big.Int, flags BehaviorFlags) error {
	...
	//Actinium: disable max target diff check
	...
	if flags&BFNoPoWCheck != BFNoPoWCheck {
		//Actinium: deactivate PoW checks
...
// difficulty is in min/max range and that the block hash is less than the
// target difficulty as claimed.
func CheckProofOfWork(block *acmutil.Block, powLimit *big.Int) error {
...
// txscript.
func CountSigOps(tx *acmutil.Tx) int {
	msgTx := tx.MsgTx()
...
func CountP2SHSigOps(tx *acmutil.Tx, isCoinBaseTx bool, utxoView *UtxoViewpoint) (int, error) {
...
func checkBlockSanity(block *acmutil.Block, powLimit *big.Int, timeSource MedianTimeSource, flags BehaviorFlags) error {
...
// CheckBlockSanity performs some preliminary checks on a block to ensure it is
// sane before continuing with block processing.  These checks are context free.
func CheckBlockSanity(block *acmutil.Block, powLimit *big.Int, timeSource MedianTimeSource) error {
...
func ExtractCoinbaseHeight(coinbaseTx *acmutil.Tx) (int32, error) {
...
func checkSerializedHeight(coinbaseTx *acmutil.Tx, wantHeight int32) error {
...
func (b *BlockChain) checkBlockHeaderContext(header *wire.BlockHeader, prevNode *blockNode, flags BehaviorFlags) error {
	...
	if !fastAdd {
		...
		//Actinium: commented out because not used (check "deactivate block diff checks" below)
		...
		//Actinium: deactivate block diff checks
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkBlockContext(block *acmutil.Block, prevNode *blockNode, flags BehaviorFlags) error {
...
func (b *BlockChain) checkBIP0030(node *blockNode, block *acmutil.Block, view *UtxoViewpoint) error {
...
func CheckTransactionInputs(tx *acmutil.Tx, txHeight int32, utxoView *UtxoViewpoint, chainParams *chaincfg.Params) (int64, error) {
	...
	for txInIndex, txIn := range tx.MsgTx().TxIn {
		...
		if originTxSatoshi < 0 {
			str := fmt.Sprintf("transaction output has negative "+
				"value of %v", acmutil.Amount(originTxSatoshi))
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		if originTxSatoshi > acmutil.MaxSatoshi {
			str := fmt.Sprintf("transaction output value of %v is "+
				"higher than max allowed value of %v",
				acmutil.Amount(originTxSatoshi),
				acmutil.MaxSatoshi)
			return 0, ruleError(ErrBadTxOutValue, str)
		}
		...
		if totalSatoshiIn < lastSatoshiIn ||
			totalSatoshiIn > acmutil.MaxSatoshi {
			str := fmt.Sprintf("total value of all transaction "+
...
//
// This function MUST be called with the chain state lock held (for writes).
func (b *BlockChain) checkConnectBlock(node *blockNode, block *acmutil.Block, view *UtxoViewpoint, stxos *[]SpentTxOut) error {
...
func (b *BlockChain) CheckConnectBlockTemplate(block *acmutil.Block) error {
	b.chainLock.Lock()
