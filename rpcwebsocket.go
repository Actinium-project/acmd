...
import (
	...
	"github.com/Actinium-project/acmd/database"
...
// processing.
func (m *wsNotificationManager) NotifyBlockConnected(block *acmutil.Block) {
...
func (m *wsNotificationManager) NotifyBlockDisconnected(block *acmutil.Block) {
...
func (m *wsNotificationManager) NotifyMempoolTx(tx *acmutil.Tx, isNew bool) {
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) addAddress(a acmutil.Address) {
	switch a := a.(type) {
...
func (f *wsClientFilter) addAddressStr(s string, params *chaincfg.Params) {
	...
	// script.
	a, err := acmutil.DecodeAddress(s, params)
	if err != nil {
		return
	}
	f.addAddress(a)
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) existsAddress(a acmutil.Address) bool {
	switch a := a.(type) {
	case *acmutil.AddressPubKeyHash:
...
//
// NOTE: This extension was ported from github.com/decred/dcrd
func (f *wsClientFilter) removeAddress(a acmutil.Address) {
	switch a := a.(type) {
	case *acmutil.AddressPubKeyHash:
...
// Notification types
type notificationBlockConnected acmutil.Block
type notificationBlockDisconnected acmutil.Block
...
func (m *wsNotificationManager) subscribedClients(tx *acmutil.Tx,
...
func (*wsNotificationManager) notifyBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := acmjson.NewBlockConnectedNtfn(block.Hash().String(), block.Height(),
		block.MsgBlock().Header.Timestamp.Unix())
	marshalledJSON, err := acmjson.MarshalCmd(nil, ntfn)
	if err != nil {
...
// reorganize).
func (*wsNotificationManager) notifyBlockDisconnected(clients map[chan struct{}]*wsClient, block *acmutil.Block) {
	...
	ntfn := acmjson.NewBlockDisconnectedNtfn(block.Hash().String(),
...
func (m *wsNotificationManager) notifyFilteredBlockConnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := acmjson.NewFilteredBlockConnectedNtfn(block.Height(),
...
func (*wsNotificationManager) notifyFilteredBlockDisconnected(clients map[chan struct{}]*wsClient,
	...
	ntfn := acmjson.NewFilteredBlockDisconnectedNtfn(block.Height(),
...
func (m *wsNotificationManager) notifyForNewTx(clients map[chan struct{}]*wsClient, tx *acmutil.Tx) {
	...
	ntfn := acmjson.NewTxAcceptedNtfn(txHashStr, acmutil.Amount(amount).ToBTC())
	marshalledJSON, err := acmjson.MarshalCmd(nil, ntfn)
	if err != nil {
	...
	var verboseNtfn *acmjson.TxAcceptedVerboseNtfn
	...
	for _, wsc := range clients {
		if wsc.verboseTxUpdates {
			...
			verboseNtfn = acmjson.NewTxAcceptedVerboseNtfn(*rawTx)
			marshalledJSONVerbose, err = acmjson.MarshalCmd(nil,
				verboseNtfn)
			if err != nil {
...
func (m *wsNotificationManager) addSpentRequests(opMap map[wire.OutPoint]map[chan struct{}]*wsClient,
	...
	spends := make(map[chainhash.Hash]*acmutil.Tx)
	for _, op := range ops {
...
func blockDetails(block *acmutil.Block, txIndex int) *acmjson.BlockDetails {
	if block == nil {
		return nil
	}
	return &acmjson.BlockDetails{
		Height: block.Height(),
...
func newRedeemingTxNotification(txHex string, index int, block *acmutil.Block) ([]byte, error) {
	// Create and marshal the notification.
	ntfn := acmjson.NewRedeemingTxNtfn(txHex, blockDetails(block, index))
	return acmjson.MarshalCmd(nil, ntfn)
}
...
func (m *wsNotificationManager) notifyForTxOuts(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	addrs map[string]map[chan struct{}]*wsClient, tx *acmutil.Tx, block *acmutil.Block) {
	...
	for i, txOut := range tx.MsgTx().TxOut {
		...
		for _, txAddr := range txAddrs {
			...
			ntfn := acmjson.NewRecvTxNtfn(txHex, blockDetails(block,
				tx.Index()))
...
// notifications.
func (m *wsNotificationManager) notifyRelevantTxAccepted(tx *acmutil.Tx,
	clients map[chan struct{}]*wsClient) {

	clientsToNotify := m.subscribedClients(tx, clients)

	if len(clientsToNotify) != 0 {
		n := acmjson.NewRelevantTxAcceptedNtfn(txHexString(tx.MsgTx()))
		marshalled, err := acmjson.MarshalCmd(nil, n)
		if err != nil {
...
func (m *wsNotificationManager) notifyForTxIns(ops map[wire.OutPoint]map[chan struct{}]*wsClient,
	tx *acmutil.Tx, block *acmutil.Block) {
...
out:
	for {
		...
		if err != nil {
			...
			jsonErr := &acmjson.RPCError{
				Code:    acmjson.ErrRPCParse.Code,
		...
		switch authCmd, ok := cmd.cmd.(*acmjson.AuthenticateCmd); {
		...
		if !c.isAdmin {
			if _, ok := rpcLimited[request.Method]; !ok {
				jsonErr := &acmjson.RPCError{
					Code:    acmjson.ErrRPCInvalidParams.Code,
...
func handleWebsocketHelp(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*acmjson.HelpCmd)
	if !ok {
		return nil, acmjson.ErrRPCInternal
	}
...
func handleLoadTxFilter(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd := icmd.(*acmjson.LoadTxFilterCmd)
...
func handleSession(wsc *wsClient, icmd interface{}) (interface{}, error) {
	return &acmjson.SessionResult{SessionID: wsc.sessionID}, nil
}
...
func handleNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*acmjson.NotifySpentCmd)
	if !ok {
		return nil, acmjson.ErrRPCInternal
	}
...
func handleNotifyNewTransactions(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*acmjson.NotifyNewTransactionsCmd)
	if !ok {
		return nil, acmjson.ErrRPCInternal
	}
...
func handleNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*acmjson.NotifyReceivedCmd)
	if !ok {
		return nil, acmjson.ErrRPCInternal
	}
...
func handleStopNotifySpent(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*acmjson.StopNotifySpentCmd)
	if !ok {
		return nil, acmjson.ErrRPCInternal
	}
...
func handleStopNotifyReceived(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*acmjson.StopNotifyReceivedCmd)
	if !ok {
		return nil, acmjson.ErrRPCInternal
	}
...
func checkAddressValidity(addrs []string, params *chaincfg.Params) error {
	for _, addr := range addrs {
		_, err := acmutil.DecodeAddress(addr, params)
		if err != nil {
			return &acmjson.RPCError{
				Code: acmjson.ErrRPCInvalidAddressOrKey,
...
func deserializeOutpoints(serializedOuts []acmjson.OutPoint) ([]*wire.OutPoint, error) {
...
var ErrRescanReorg = acmjson.RPCError{
	Code:    acmjson.ErrRPCDatabase,
	Message: "Reorganize",
}

// rescanBlock rescans all transactions in a single block.  This is a helper
// function for handleRescan.
func rescanBlock(wsc *wsClient, lookups *rescanKeys, blk *acmutil.Block) {
	for _, tx := range blk.Transactions() {
		...
		for txOutIdx, txout := range tx.MsgTx().TxOut {
			...
			for _, addr := range addrs {
				...
				ntfn := acmjson.NewRecvTxNtfn(txHex,
...
//
// NOTE: This extension is ported from github.com/decred/dcrd
func rescanBlockFilter(filter *wsClientFilter, block *acmutil.Block, params *chaincfg.Params) []string {
...
func handleRescanBlocks(wsc *wsClient, icmd interface{}) (interface{}, error) {
	cmd, ok := icmd.(*acmjson.RescanBlocksCmd)
	if !ok {
		return nil, acmjson.ErrRPCInternal
	}
	...
	discoveredData := make([]acmjson.RescannedBlock, 0, len(blockHashes))
	...
	for i := range blockHashes {
		...
		if err != nil {
			return nil, &acmjson.RPCError{
				Code:    acmjson.ErrRPCBlockNotFound,
		...
		if len(transactions) != 0 {
			discoveredData = append(discoveredData, acmjson.RescannedBlock{
...
func descendantBlock(prevHash *chainhash.Hash, curBlock *acmutil.Block) error {
...
func scanBlockChunks(wsc *wsClient, cmd *acmjson.RescanCmd, lookups *rescanKeys, minBlock,
	maxBlock int32, chain *blockchain.BlockChain) (
	*acmutil.Block, *chainhash.Hash, error) {
...
fetchRange:
	for minBlock < maxBlock {
		...
		if err != nil {
			rpcsLog.Errorf("Error looking up block range: %v", err)
			return nil, nil, &acmjson.RPCError{
				Code:    acmjson.ErrRPCDatabase,
				Message: "Database error: " + err.Error(),
			}
		}
	...
	loopHashList:
		for i := range hashList {
			...
			n := acmjson.NewRescanProgressNtfn(
				hashList[i].String(), blk.Height(),
				blk.MsgBlock().Header.Timestamp.Unix(),
			)
			mn, err := acmjson.MarshalCmd(nil, n)
			if err != nil {
