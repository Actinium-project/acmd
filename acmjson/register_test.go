...
func TestUsageFlagStringer(t *testing.T) {
	...
	tests := []struct {
		in   acmjson.UsageFlag
		want string
	}{
		{0, "0x0"},
		{acmjson.UFWalletOnly, "UFWalletOnly"},
		{acmjson.UFWebsocketOnly, "UFWebsocketOnly"},
		{acmjson.UFNotification, "UFNotification"},
		{acmjson.UFWalletOnly | acmjson.UFWebsocketOnly,
			"UFWalletOnly|UFWebsocketOnly"},
		{acmjson.UFWalletOnly | acmjson.UFWebsocketOnly | (1 << 31),
			"UFWalletOnly|UFWebsocketOnly|0x80000000"},
	}

	// Detect additional usage flags that don't have the stringer added.
	numUsageFlags := 0
	highestUsageFlagBit := acmjson.TstHighestUsageFlagBit
...
func TestRegisterCmdErrors(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			err: acmjson.Error{ErrorCode: acmjson.ErrDuplicateMethod},
		},
		{
			...
			flags: acmjson.TstHighestUsageFlagBit,
			err:   acmjson.Error{ErrorCode: acmjson.ErrInvalidUsageFlags},
		},
		{
			name:   "invalid type",
			method: "registertestcmd",
		...
		},
		{
			...
			err: acmjson.Error{ErrorCode: acmjson.ErrEmbeddedType},
		},
		{
			...
			err: acmjson.Error{ErrorCode: acmjson.ErrUnexportedField},
		},
		{
			...
			err: acmjson.Error{ErrorCode: acmjson.ErrUnsupportedFieldType},
		},
		{
		...
		},
		{
			...
			err: acmjson.Error{ErrorCode: acmjson.ErrNonOptionalField},
		},
		{
			...
			err: acmjson.Error{ErrorCode: acmjson.ErrNonOptionalDefault},
		},
		{
			...
			err: acmjson.Error{ErrorCode: acmjson.ErrMismatchedDefault},
		},
	}

	t.Logf("Running %d tests", len(tests))
	for i, test := range tests {
		err := acmjson.RegisterCmd(test.method, test.cmdFunc(),
			test.flags)
...
func TestMustRegisterCmdPanic(t *testing.T) {
	...
	acmjson.MustRegisterCmd("panicme", 0, 0)
}
