...
func TestIsValidIDType(t *testing.T) {
	...
	for i, test := range tests {
		if acmjson.IsValidIDType(test.id) != test.isValid {
...
func TestMarshalResponse(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			result: nil,
			jsonErr: func() *acmjson.RPCError {
				return acmjson.NewRPCError(acmjson.ErrRPCBlockNotFound, "123 not found")
			}(),
	...
	for i, test := range tests {
		_, _ = i, test
		marshalled, err := acmjson.MarshalResponse(testID, test.result, test.jsonErr)
		if err != nil {
...
func TestMiscErrors(t *testing.T) {
	...
	// not supported.
	_, err := acmjson.NewRequest(nil, "test", []interface{}{make(chan int)})
	if err == nil {
	...
	// supported.
	wantErr := acmjson.Error{ErrorCode: acmjson.ErrInvalidType}
	_, err = acmjson.MarshalResponse(make(chan int), nil, nil)
	if jerr, ok := err.(acmjson.Error); !ok || jerr.ErrorCode != wantErr.ErrorCode {
	...
	// can't be marshalled.
	_, err = acmjson.MarshalResponse(1, make(chan int), nil)
...
func TestRPCError(t *testing.T) {
	...
	tests := []struct {
		...
		{
			acmjson.ErrRPCInvalidRequest,
			"-32600: Invalid request",
		},
		{
			acmjson.ErrRPCMethodNotFound,
