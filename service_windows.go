...
import (
	...
	"github.com/Actinium-project/winsvc/eventlog"
	"github.com/Actinium-project/winsvc/mgr"
	"github.com/Actinium-project/winsvc/svc"
)

const (
	// svcName is the name of acmd service.
	svcName = "acmdsvc"
...
// logServiceStartOfDay logs information about acmd when the main server has
...
// acmdService houses the main service handler which handles all service
// updates and launching acmdMain.
type acmdService struct{}
...
// long-running acmdMain (which is the real meat of acmd), handles service
// change requests, and notifies the service control manager of changes.
func (s *acmdService) Execute(args []string, r <-chan svc.ChangeRequest, changes chan<- svc.Status) (bool, uint32) {
	// Service start is pending.
	const cmdsAccepted = svc.AcceptStop | svc.AcceptShutdown
	changes <- svc.Status{State: svc.StartPending}

	// Start acmdMain in a separate goroutine so the service can start
...
// installService attempts to install the acmd service.  Typically this should
...
// removeService attempts to uninstall the acmd service.  Typically this should
...
// startService attempts to start the acmd service.
...
func serviceMain() (bool, error) {
	...
	err = svc.Run(svcName, &acmdService{})
	if err != nil {
