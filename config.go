...
import (
	...
	_ "github.com/Actinium-project/acmd/database/ffldb"
	"github.com/Actinium-project/acmd/mempool"
...
const (
	...
	sampleConfigFilename         = "sample-acmd.conf"
	defaultTxIndex               = false
	defaultAddrIndex             = false
)

var (
	defaultHomeDir     = acmutil.AppDataDir("acmd", false)
...
// config defines the configuration options for acmd.
//
...
type config struct {
	...
	MinRelayTxFee        float64       `long:"minrelaytxfee" description:"The minimum transaction fee in ACM/kB to be considered a non-zero fee."`
...
//
// The above results in acmd functioning properly without any config settings
...
func loadConfig() (*config, []string, error) {
	...
	if cfg.DisableRPC {
		acmdLog.Infof("RPC service is disabled")
	}
	...
	cfg.minRelayTxFee, err = acmutil.NewAmount(cfg.MinRelayTxFee)
	if err != nil {
	...
	cfg.miningAddrs = make([]acmutil.Address, 0, len(cfg.MiningAddrs))
	for _, strAddr := range cfg.MiningAddrs {
		addr, err := acmutil.DecodeAddress(strAddr, activeNetParams.Params)
		if err != nil {
	...
	if configFileError != nil {
		acmdLog.Warnf("%v", configFileError)
	}

	return &cfg, remainingArgs, nil
}

// createDefaultConfig copies the file sample-acmd.conf to the given destination path,
...
// acmdDial connects to the address on the named network using the appropriate
// dial function depending on the address and configuration options.  For
// example, .onion addresses will be dialed using the onion specific proxy if
// one was specified, but will otherwise use the normal dial function (which
// could itself use a proxy or not).
func acmdDial(addr net.Addr) (net.Conn, error) {
...
// acmdLookup resolves the IP of the given host using the correct DNS lookup
...
func acmdLookup(host string) ([]net.IP, error) {
