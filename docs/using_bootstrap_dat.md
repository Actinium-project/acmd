...
5. [How do I use bootstrap.dat with acmd?](#Importing)
...
### 5. How do I use bootstrap.dat with acmd?

acmd comes with a separate utility named `addblock` which can be used to import
...
1. Stop acmd if it is already running.  This is required since addblock needs to
   access the database used by acmd and it will be locked if acmd is using it.
