...
acmd provides full support for anonymous networking via the
...
[stream isolation](#TorStreamIsolation).  In addition, acmd supports a hybrid,
...
Configuring acmd as a Tor client is straightforward.  The first step is
obviously to install Tor and ensure it is working. Once that is done, all that
typically needs to be done is to specify the `--proxy` flag via the acmd command
line or in the acmd configuration file.  Typically the Tor proxy address will be
...
By default, acmd assumes the proxy specified with `--proxy` is a Tor proxy and
...
discuss since acmd will act as the server.
...
```text
HiddenServiceDir /var/tor/acmd
HiddenServicePort 9333 127.0.0.1:9333
```

Once Tor is configured to provide the hidden service and you have obtained your
generated .onion address, configuring acmd as a Tor hidden service requires
three flags:
...
```bash
$ ./acmd --proxy=127.0.0.1:9050 --listen=127.0.0.1 --externalip=fooanon.onion
```
...
acmd provides support for operating as a bridge between regular nodes and hidden
...
by using the `--onion` flag via the acmd command line or in the acmd
...
```bash
$ ./acmd --onion=127.0.0.1:9050 --externalip=fooanon.onion
```
...
acmd provides support for Tor stream isolation by using the `--torisolation`
...
```bash
$ ./acmd --proxy=127.0.0.1:9050 --torisolation
```
