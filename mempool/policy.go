...
const (
	...
	DefaultMinRelayTxFee = acmutil.Amount(1000)
...
// pool and relayed.
func calcMinRequiredTxRelayFee(serializedSize int64, minRelayTxFee acmutil.Amount) int64 {
	...
	if minFee < 0 || minFee > acmutil.MaxSatoshi {
		minFee = acmutil.MaxSatoshi
	}
...
func checkInputsStandard(tx *acmutil.Tx, utxoView *blockchain.UtxoViewpoint) error {
...
func isDust(txOut *wire.TxOut, minRelayTxFee acmutil.Amount) bool {
...
func checkTransactionStandard(tx *acmutil.Tx, height int32,
	medianTimePast time.Time, minRelayTxFee acmutil.Amount,
...
func GetTxVirtualSize(tx *acmutil.Tx) int64 {
	// vSize := (weight(tx) + 3) / 4
