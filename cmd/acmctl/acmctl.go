...
func main() {
	...
	method := args[0]
	usageFlags, err := acmjson.MethodUsageFlags(method)
	if err != nil {
	...
	cmd, err := acmjson.NewCmd(method, params...)
	if err != nil {
		// Show the error along with its error code when it's a
		// acmjson.Error as it reallistcally will always be since the
		// NewCmd function is only supposed to return errors of that
		// type.
		if jerr, ok := err.(acmjson.Error); ok {
		...
		// The error is not a acmjson.Error and this really should not
	...
	marshalledJSON, err := acmjson.MarshalCmd(1, cmd)
	if err != nil {
