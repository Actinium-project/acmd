...
const (
	...
	unusableFlags = acmjson.UFWebsocketOnly | acmjson.UFNotification
)

var (
	acmdHomeDir           = acmutil.AppDataDir("acmd", false)
	acmctlHomeDir         = acmutil.AppDataDir("acmctl", false)
	acmwalletHomeDir      = acmutil.AppDataDir("acmwallet", false)
	defaultConfigFile     = filepath.Join(acmctlHomeDir, "acmctl.conf")
	defaultRPCServer      = "localhost"
	defaultRPCCertFile    = filepath.Join(acmdHomeDir, "rpc.cert")
	defaultWalletCertFile = filepath.Join(acmwalletHomeDir, "rpc.cert")
)
...
func listCommands() {
	...
	cmdMethods := acmjson.RegisteredCmdMethods()
	categorized := make([][]string, numCategories)
	for _, method := range cmdMethods {
		flags, err := acmjson.MethodUsageFlags(method)
		if err != nil {
		...
		usage, err := acmjson.MethodUsageText(method)
		if err != nil {
			// This should never happen since the method was just
			// returned from the package, but be safe.
			continue
		...
		if flags&acmjson.UFWalletOnly != 0 {
...
// config defines the configuration options for acmctl.
//
...
func cleanAndExpandPath(path string) string {
	...
	if strings.HasPrefix(path, "~") {
		homeDir := filepath.Dir(acmctlHomeDir)
...
func loadConfig() (*config, []string, error) {
	...
	if _, err := os.Stat(preCfg.ConfigFile); os.IsNotExist(err) {
		...
		if preCfg.Wallet {
			serverConfigPath = filepath.Join(acmwalletHomeDir, "acmwallet.conf")
		} else {
			serverConfigPath = filepath.Join(acmdHomeDir, "acmd.conf")
		}
...
// For this it tries to read the config file for the RPC server (either acmd or
// acmwallet), and extract the RPC user and password from it.
