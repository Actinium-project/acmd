...
import (
	...
	"github.com/Actinium-project/acmd/mining/cpuminer"
	"github.com/Actinium-project/acmd/netsync"
...
func (sp *serverPeer) OnTx(_ *peer.Peer, msg *wire.MsgTx) {
	...
	// Convert the raw MsgTx to a acmutil.Tx which provides some convenience
	// methods and things such as hash caching.
	tx := acmutil.NewTx(msg)
...
func (sp *serverPeer) OnBlock(_ *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	// Convert the raw MsgBlock to a acmutil.Block which provides some
	// convenience methods and things such as hash caching.
	block := acmutil.NewBlockFromBlockAndBytes(msg, buf)
...
func (s *server) TransactionConfirmed(tx *acmutil.Tx) {
...
func (s *server) peerHandler() {
	...
	if !cfg.DisableDNSSeed {
		...
		connmgr.SeedFromDNS(activeNetParams.Params, defaultRequiredServices,
			acmdLookup, func(addrs []*wire.NetAddress) {
...
// newServer returns a new acmd server configured to listen on addr for the
...
func newServer(listenAddrs, agentBlacklist, agentWhitelist []string,
	...
	amgr := addrmgr.New(cfg.DataDir, acmdLookup)
	...
	txC := mempool.Config{
		...
		CalcSequenceLock: func(tx *acmutil.Tx, view *blockchain.UtxoViewpoint) (*blockchain.SequenceLock, error) {
