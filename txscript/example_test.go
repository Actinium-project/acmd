...
func ExamplePayToAddrScript() {
	// Parse the address to send the coins to into a acmutil.Address
	// which is useful to ensure the accuracy of the address and determine
	// the address type.  It is also required for the upcoming call to
	// PayToAddrScript.
	addressStr := "Leqay1p3dWuprsnwUjM2SBbrH9BiphaDUL"
	address, err := acmutil.DecodeAddress(addressStr, &chaincfg.MainNetParams)
	if err != nil {
		fmt.Println(err)
		return
	}
...
func ExampleSignTxOutput() {
	...
	addr, err := acmutil.NewAddressPubKeyHash(pubKeyHash,
	...
	// contains a single output that pays to address in the amount of 1 ACM.
	...
	lookupKey := func(a acmutil.Address) (*btcec.PrivateKey, bool, error) {
