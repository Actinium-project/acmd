...
// newAddressPubKey returns a new acmutil.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) acmutil.Address {
	addr, err := acmutil.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new acmutil.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) acmutil.Address {
	addr, err := acmutil.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new acmutil.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) acmutil.Address {
	addr, err := acmutil.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the acmutil.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the acmutil.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2pkhMain, err := acmutil.NewAddressPubKeyHash(hexToBytes("e34cce70c86"+
	...
	p2shMain, _ := acmutil.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := acmutil.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := acmutil.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := acmutil.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*acmutil.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*acmutil.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*acmutil.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*acmutil.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*acmutil.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
