...
// Harness fully encapsulates an active acmd process to provide a unified
// platform for creating rpc driven integration tests involving acmd. The
// active acmd node will typically be run in simnet mode in order to allow for
// easy generation of test blockchains.  The active acmd process is fully
...
func New(activeNet *chaincfg.Params, handlers *rpcclient.NotificationHandlers,
	...
	if handlers.OnFilteredBlockConnected != nil {
		...
		handlers.OnFilteredBlockConnected = func(height int32, header *wire.BlockHeader, filteredTxns []*acmutil.Tx) {
...
func (h *Harness) SetUp(createTestChain bool, numMatureOutputs uint32) error {
	// Start the acmd node itself. This spawns a new process which will be
	// managed
	...
	// wallet.
	filterAddrs := []acmutil.Address{h.wallet.coinbaseAddr}
	if err := h.Node.LoadTxFilter(true, filterAddrs, nil); err != nil {
		return err
	}

	// Ensure acmd properly dispatches our registered call-back for each new
...
// connectRPCClient attempts to establish an RPC connection to the created acmd
...
func (h *Harness) NewAddress() (acmutil.Address, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) ConfirmedBalance() acmutil.Amount {
...
func (h *Harness) SendOutputs(targetOutputs []*wire.TxOut,
	feeRate acmutil.Amount) (*chainhash.Hash, error) {
...
func (h *Harness) CreateTransaction(targetOutputs []*wire.TxOut,
	feeRate acmutil.Amount, change bool) (*wire.MsgTx, error) {
...
//
// This function is safe for concurrent access.
func (h *Harness) GenerateAndSubmitBlock(txns []*acmutil.Tx, blockVersion int32,
	blockTime time.Time) (*acmutil.Block, error) {
...
func (h *Harness) GenerateAndSubmitBlockWithCustomCoinbaseOutputs(
	txns []*acmutil.Tx, blockVersion int32, blockTime time.Time,
	mineTo []wire.TxOut) (*acmutil.Block, error) {
	...
	prevBlock := acmutil.NewBlock(mBlock)
...
func baseDir() (string, error) {
	dirPath := filepath.Join(os.TempDir(), "acmd", "rpctest")
