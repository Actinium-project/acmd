...
func testSendOutputs(r *Harness, t *testing.T) {
	genSpend := func(amt acmutil.Amount) *chainhash.Hash {
		...
		// Next, send amt ACM to this address, spending from one of our mature
		// coinbase outputs.
	...
	// input.
	txid := genSpend(acmutil.Amount(5 * acmutil.SatoshiPerBitcoin))
	...
	txid = genSpend(acmutil.Amount(500 * acmutil.SatoshiPerBitcoin))
	blockHashes, err = r.Node.Generate(1)
	if err != nil {
		t.Fatalf("unable to generate single block: %v", err)
	}
...
func testGenerateAndSubmitBlock(r *Harness, t *testing.T) {
	...
	output := wire.NewTxOut(acmutil.SatoshiPerBitcoin, pkScript)

	const numTxns = 5
	txns := make([]*acmutil.Tx, 0, numTxns)
	for i := 0; i < numTxns; i++ {
		...
		txns = append(txns, acmutil.NewTx(tx))
	}
...
func testMemWalletReorg(r *Harness, t *testing.T) {
	...
	// The internal wallet of this harness should now have 250 ACM.
	expectedBalance := acmutil.Amount(250 * acmutil.SatoshiPerBitcoin)
	...
	// The original wallet should now have a balance of 0 ACM as its entire
	// chain should have been decimated in favor of the main harness'
	// chain.
	expectedBalance = acmutil.Amount(0)
	walletBalance = harness.ConfirmedBalance()
	if expectedBalance != walletBalance {
		t.Fatalf("wallet balance incorrect: expected %v, got %v",
			expectedBalance, walletBalance)
...
func testMemWalletLockedOutputs(r *Harness, t *testing.T) {
	...
	outputAmt := acmutil.Amount(50 * acmutil.SatoshiPerBitcoin)
	...
	// The current wallet balance should now be at least 50 ACM less
...
func TestHarness(t *testing.T) {
	// We should have (numMatureOutputs * 50 ACM) of mature unspendable
	// outputs.
	expectedBalance := acmutil.Amount(numMatureOutputs * 50 * acmutil.SatoshiPerBitcoin)
