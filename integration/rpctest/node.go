...
// nodeConfig contains all the args, and data required to launch a acmd process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	acmdPath, err := acmdExecutablePath()
	if err != nil {
		acmdPath = "acmd"
	}
...
// arguments returns an array of arguments that be used to launch the acmd
// process.
...
// command returns the exec.Cmd which will be used to start the acmd process.
...
// to the acmd process that is launched via Start().
...
// as the base for the log and data directories for acmd.
...
// start creates a new acmd process, and writes its pid in a file reserved for
...
// stop interrupts the running acmd process process, and waits until it exits
...
// shutdown terminates the running acmd process, and cleans up all
...
func genCertPair(certFile, keyFile string) error {
	...
	cert, key, err := acmutil.NewTLSCertPair(org, validUntil, nil)
	if err != nil {
		return err
	}
