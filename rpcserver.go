...
var (
	...
	// data.
	gbtCoinbaseAux = &acmjson.GetBlockTemplateResultAux{
...
var (
	...
	ErrRPCUnimplemented = &acmjson.RPCError{
		Code:    acmjson.ErrRPCUnimplemented,
		Message: "Command unimplemented",
	}

	// ErrRPCNoWallet is an error returned to RPC clients when the provided
	// command is recognized as a wallet command.
	ErrRPCNoWallet = &acmjson.RPCError{
		Code:    acmjson.ErrRPCNoWallet,
...
// list of commands that we recognize, but for which acmd has no support because
// it lacks support for wallet functionality. For these commands the user
// should ask a connected instance of acmwallet.
...
// not needed.
func internalRPCError(errStr, context string) *acmjson.RPCError {
	logStr := errStr
	if context != "" {
		logStr = context + ": " + errStr
	}
	rpcsLog.Error(logStr)
	return acmjson.NewRPCError(acmjson.ErrRPCInternal.Code, errStr)
}

// rpcDecodeHexError is a convenience function for returning a nicely formatted
// RPC error which indicates the provided hex string failed to decode.
func rpcDecodeHexError(gotHex string) *acmjson.RPCError {
	return acmjson.NewRPCError(acmjson.ErrRPCDecodeHexString,
...
// transaction hash.
func rpcNoTxInfoError(txHash *chainhash.Hash) *acmjson.RPCError {
	return acmjson.NewRPCError(acmjson.ErrRPCNoTxInfo,
...
// These commands will be implemented in acmwallet.
...
func handleCreateRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.CreateRawTransactionCmd)
	...
	for encodedAddr, amount := range c.Amounts {
		...
		if amount <= 0 || amount > acmutil.MaxSatoshi {
			return nil, &acmjson.RPCError{
				Code:    acmjson.ErrRPCType,
				Message: "Invalid amount",
			}
		}

		// Decode the provided address.
		addr, err := acmutil.DecodeAddress(encodedAddr, params)
		if err != nil {
			return nil, &acmjson.RPCError{
		...
		satoshi, err := acmutil.NewAmount(amount)
		if err != nil {
...
// transaction.
func createVinList(mtx *wire.MsgTx) []acmjson.Vin {
	// Coinbase transactions only have a single txin by definition.
	vinList := make([]acmjson.Vin, len(mtx.TxIn))
	...
	for i, txIn := range mtx.TxIn {
		...
		vinEntry.ScriptSig = &acmjson.ScriptSig{
			Asm: disbuf,
...
// transaction.
func createVoutList(mtx *wire.MsgTx, chainParams *chaincfg.Params, filterAddrMap map[string]struct{}) []acmjson.Vout {
	voutList := make([]acmjson.Vout, 0, len(mtx.TxOut))
	for i, v := range mtx.TxOut {
		...
		var vout acmjson.Vout
		vout.N = uint32(i)
		vout.Value = acmutil.Amount(v.Value).ToBTC()
...
func createTxRawResult(chainParams *chaincfg.Params, mtx *wire.MsgTx,
	...
	blkHeight int32, chainHeight int32) (*acmjson.TxRawResult, error) {
	...
	txReply := &acmjson.TxRawResult{
		Hex:      mtxHex,
		Txid:     txHash,
		Hash:     mtx.WitnessHash().String(),
		Size:     int32(mtx.SerializeSize()),
		Vsize:    int32(mempool.GetTxVirtualSize(acmutil.NewTx(mtx))),
		Weight:   int32(blockchain.GetTransactionWeight(acmutil.NewTx(mtx))),
...
func handleDecodeRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.DecodeRawTransactionCmd)
	...
	if err != nil {
		return nil, &acmjson.RPCError{
			Code:    acmjson.ErrRPCDeserialization,
			Message: "TX decode failed: " + err.Error(),
		}
	}

	// Create and return the result.
	txReply := acmjson.TxRawDecodeResult{
...
func handleDecodeScript(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	p2sh, err := acmutil.NewAddressScriptHash(script, s.cfg.ChainParams)
	if err != nil {
	...
	reply := acmjson.DecodeScriptResult{
		Asm:       disbuf,
...
func handleGenerate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if len(cfg.miningAddrs) == 0 {
		return nil, &acmjson.RPCError{
			Code: acmjson.ErrRPCInternal.Code,
...
func handleGetAddedNodeInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.GetAddedNodeInfoCmd)
	...
	if c.Node != nil {
		...
		if !found {
			return nil, &acmjson.RPCError{
				Code:    acmjson.ErrRPCClientNodeNotAdded,
	...
	results := make([]*acmjson.GetAddedNodeInfoResult, 0, len(peers))
	for _, rpcPeer := range peers {
		// Set the "address" of the peer which could be an ip address
		// or a domain name.
		peer := rpcPeer.ToPeer()
		var result acmjson.GetAddedNodeInfoResult
		result.AddedNode = peer.Addr()
		result.Connected = acmjson.Bool(peer.Connected())
		...
		addrs := make([]acmjson.GetAddedNodeInfoResultAddr, 0, len(ipList))
		for _, ip := range ipList {
			var addr acmjson.GetAddedNodeInfoResultAddr
			addr.Address = ip
...
func handleGetBestBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	result := &acmjson.GetBestBlockResult{
...
func handleGetBlock(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	blk, err := acmutil.NewBlockFromBytes(blkBytes)
	if err != nil {
	...
	blockReply := acmjson.GetBlockVerboseResult{
		Hash:          c.Hash,
	...
	if c.VerboseTx == nil || !*c.VerboseTx {
		...
		rawTxns := make([]acmjson.TxRawResult, len(txns))
		for i, tx := range txns {
...
func handleGetBlockChainInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	chainInfo := &acmjson.GetBlockChainInfoResult{
		Chain:         params.Name,
		...
		Pruned:        false,
		SoftForks: &acmjson.SoftForks{
			Bip9SoftForks: make(map[string]*acmjson.Bip9SoftForkDescription),
		},
	}
	...
	chainInfo.SoftForks.SoftForks = []*acmjson.SoftForkDescription{
		{
			ID:      "bip34",
			Version: 2,
			Reject: struct {
	...
	for deployment, deploymentDetails := range params.Deployments {
		...
		chainInfo.SoftForks.Bip9SoftForks[forkName] = &acmjson.Bip9SoftForkDescription{
...
func handleGetBlockHash(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.GetBlockHashCmd)
	hash, err := s.cfg.Chain.BlockHashByHeight(int32(c.Index))
	if err != nil {
		return nil, &acmjson.RPCError{
			Code:    acmjson.ErrRPCOutOfRange,
...
func handleGetBlockHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.GetBlockHeaderCmd)
	...
	params := s.cfg.ChainParams
	blockHeaderReply := acmjson.GetBlockHeaderVerboseResult{
		Hash:          c.Hash,
		Confirmations: int64(1 + best.Height - blockHeight),
		Height:        blockHeight,
		Version:       blockHeader.Version,
...
// state as a acmjson.GetBlockTemplateResult that is ready to be encoded to JSON
// and returned to the caller.
//
// This function MUST be called with the state locked.
func (state *gbtWorkState) blockTemplateResult(useCoinbaseValue bool, submitOld *bool) (*acmjson.GetBlockTemplateResult, error) {
	...
	transactions := make([]acmjson.GetBlockTemplateResultTx, 0, numTx-1)
	...
	for i, tx := range msgBlock.Transactions {
		...
		bTx := acmutil.NewTx(tx)
		resultTx := acmjson.GetBlockTemplateResultTx{
	...
	reply := acmjson.GetBlockTemplateResult{
...
func handleGetBlockTemplateRequest(s *rpcServer, request *acmjson.TemplateRequest, closeChan <-chan struct{}) (interface{}, error) {
	...
	if !(cfg.RegressionTest || cfg.SimNet) &&
		...
		return nil, &acmjson.RPCError{
			Code:    acmjson.ErrRPCClientNotConnected,
	...
	if currentHeight != 0 && !s.cfg.SyncMgr.IsCurrent() {
		return nil, &acmjson.RPCError{
			Code:    acmjson.ErrRPCClientInInitialDownload,
...
func handleGetBlockTemplateProposal(s *rpcServer, request *acmjson.TemplateRequest) (interface{}, error) {
	hexData := request.Data
	if hexData == "" {
		return false, &acmjson.RPCError{
			Code: acmjson.ErrRPCType,
...
func handleGetBlockTemplate(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.GetBlockTemplateCmd)
	request := c.Request
...
func handleGetCFilter(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	if s.cfg.CfIndex == nil {
		return nil, &acmjson.RPCError{
			Code:    acmjson.ErrRPCNoCFIndex,
			Message: "The CF index must be enabled for this command",
		}
	}

	c := cmd.(*acmjson.GetCFilterCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetCFilterHeader(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*acmjson.GetCFilterHeaderCmd)
	hash, err := chainhash.NewHashFromStr(c.Hash)
	if err != nil {
		return nil, rpcDecodeHexError(c.Hash)
	}
...
func handleGetMempoolInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	ret := &acmjson.GetMempoolInfoResult{
...
func handleGetMiningInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	gnhpsCmd := acmjson.NewGetNetworkHashPSCmd(nil, nil)
	...
	best := s.cfg.Chain.BestSnapshot()
	result := acmjson.GetMiningInfoResult{
...
func handleGetNetTotals(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	reply := &acmjson.GetNetTotalsResult{
...
func handleGetNetworkHashPS(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*acmjson.GetNetworkHashPSCmd)
...
func handleGetPeerInfo(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	infos := make([]*acmjson.GetPeerInfoResult, 0, len(peers))
	for _, p := range peers {
		statsSnap := p.ToPeer().StatsSnapshot()
		info := &acmjson.GetPeerInfoResult{
			ID:             statsSnap.ID,
			Addr:           statsSnap.Addr,
...
func handleGetRawMempool(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.GetRawMempoolCmd)
	mp := s.cfg.TxMemPool
...
func handleGetRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.GetRawTransactionCmd)
...
func handleGetTxOut(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	if includeMempool && s.cfg.TxMemPool.HaveTransaction(txHash) {
		...
		if c.Vout > uint32(len(mtx.TxOut)-1) {
			return nil, &acmjson.RPCError{
				Code: acmjson.ErrRPCInvalidTxVout,
	...
	txOutReply := &acmjson.GetTxOutResult{
		BestBlock:     bestBlockHash,
		Confirmations: int64(confirmations),
		Value:         acmutil.Amount(value).ToBTC(),
		ScriptPubKey: acmjson.ScriptPubKeyResult{
			Asm:       disbuf,
...
// passed transaction.
func createVinListPrevOut(s *rpcServer, mtx *wire.MsgTx, chainParams *chaincfg.Params, vinExtra bool, filterAddrMap map[string]struct{}) ([]acmjson.VinPrevOut, error) {
	// Coinbase transactions only have a single txin by definition.
	if blockchain.IsCoinBaseTx(mtx) {
		...
		txIn := mtx.TxIn[0]
		vinList := make([]acmjson.VinPrevOut, 1)
		vinList[0].Coinbase = hex.EncodeToString(txIn.SignatureScript)
		vinList[0].Sequence = txIn.Sequence
		return vinList, nil
	}
	...
	vinList := make([]acmjson.VinPrevOut, 0, len(mtx.TxIn))
	...
	for _, txIn := range mtx.TxIn {
		...
		if vinExtra {
			...
			vinListEntry.PrevOut = &acmjson.PrevOut{
				Addresses: encodedAddrs,
				Value:     acmutil.Amount(originTxOut.Value).ToBTC(),
			}
		}
	}
...
func fetchMempoolTxnsForAddress(s *rpcServer, addr acmutil.Address, numToSkip, numRequested uint32) ([]*acmutil.Tx, uint32) {
...
func handleSearchRawTransactions(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	...
	c := cmd.(*acmjson.SearchRawTransactionsCmd)
	vinExtra := false
	if c.VinExtra != nil {
	...
	params := s.cfg.ChainParams
	addr, err := acmutil.DecodeAddress(c.Address, params)
	if err != nil {
		return nil, &acmjson.RPCError{
			Code:    acmjson.ErrRPCInvalidAddressOrKey,
			Message: "Invalid address or key: " + err.Error(),
	...
	// The verbose flag is set, so generate the JSON object and return it.
	best := s.cfg.Chain.BestSnapshot()
	srtList := make([]acmjson.SearchRawTransactionsResult, len(addressTxns))
	for i := range addressTxns {
...
func handleSendRawTransaction(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.SendRawTransactionCmd)
	...
	if err != nil {
		...
		if txRuleErr, ok := ruleErr.Err.(mempool.TxRuleError); ok {
			...
			case strings.Contains(
				...
				code = acmjson.ErrRPCTxAlreadyInChain

			default:
				code = acmjson.ErrRPCTxRejected
			}
		}
...
func handleValidateAddress(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.ValidateAddressCmd)

	result := acmjson.ValidateAddressChainResult{}
	addr, err := acmutil.DecodeAddress(c.Address, s.cfg.ChainParams)
	if err != nil {
...
func handleVerifyMessage(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	c := cmd.(*acmjson.VerifyMessageCmd)
	...
	address, err := acmutil.NewAddressPubKey(serializedPK, params)
	if err != nil {
...
func handleVersion(s *rpcServer, cmd interface{}, closeChan <-chan struct{}) (interface{}, error) {
	result := map[string]acmjson.VersionResult{
		"acmdjsonrpcapi": {
...
func (s *rpcServer) standardCmdResult(cmd *parsedRPCCmd, closeChan <-chan struct{}) (interface{}, error) {
	...
	return nil, acmjson.ErrRPCMethodNotFound
handled:
...
func parseCmd(request *acmjson.Request) *parsedRPCCmd {
	...
	if err != nil {
		...
		if jerr, ok := err.(acmjson.Error); ok &&
			jerr.ErrorCode == acmjson.ErrUnregisteredMethod {

			parsedCmd.err = acmjson.ErrRPCMethodNotFound
			return &parsedCmd
		}

		// Otherwise, some type of invalid parameters is the
		// cause, so produce the equivalent RPC error.
		parsedCmd.err = acmjson.NewRPCError(
			acmjson.ErrRPCInvalidParams.Code, err.Error())
		return &parsedCmd
	}
...
// the type *acmjson.RPCError to the appropriate type as needed.
func createMarshalledReply(id, result interface{}, replyErr error) ([]byte, error) {
	var jsonErr *acmjson.RPCError
	if replyErr != nil {
		if jErr, ok := replyErr.(*acmjson.RPCError); ok {
			jsonErr = jErr
		} else {
	...
	return acmjson.MarshalResponse(id, result, jsonErr)
}
...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="acmd RPC"`)
...
type rpcserverSyncManager interface {
	...
	SubmitBlock(block *acmutil.Block, flags blockchain.BehaviorFlags) (bool, error)
